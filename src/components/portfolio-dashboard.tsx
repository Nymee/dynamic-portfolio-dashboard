...
function SummaryCard({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      {percent !== undefined && (
        <p className={`mt-1 text-sm ${valueColor}`}>
...
export default function PortfolioDashboard({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const data = await portfolioRepository.getSectors();
      setSectors(data);
      setLastUpdated(new Date());
      setError(null);
    } catch (err) {
      setError(
        err instanceof AppError ? err.message : "Failed to refresh data"
      );
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-screen-xl space-y-8 px-6 py-8">
        {error && (
